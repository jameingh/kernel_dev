...
为了更好地理解这段代码，我们可以将其类比为高级语言中的动态函数调用（Reflection / Eval）。

### 4.1 类比对应关系

| 汇编代码 / 动作 | 核心概念 | 高级语言类比 |
| :--- | :--- | :--- |
| **`int 0x80`** | **通用执行器** | `eval(...)` 或 `syscall_dispatcher.invoke(...)` <br> CPU 的“魔法指令”，负责打断流程并转交控制权给内核。 |
| **`mov $1, %%eax`** | **方法 ID / 函数名** | 传给 `eval` 的第一个参数：`func_name = "print"` <br> 内核醒来后，查 EAX 才知道要办什么业务。 |
| **`mov %0, %%ebx`** <br> **`: "r"(".")`** | **函数参数** | 传给 `eval` 的实参列表：`args = ["."]` <br> 准备好数据，供内核读取。 |

### 4.2 伪代码示意

如果把这段汇编翻译成一种“伪高级语言”，其逻辑如下：

```python
# 汇编视角
EAX = 1          # 准备动作：选函数（系统调用号）
EBX = &(".")     # 准备动作：备参数（字符串地址）
int 0x80         # 关键动作：执行！陷入内核！

# 高级语言视角（动态调用/反射）
syscall_dispatcher.invoke(
    func_id = 1,      # 对应 EAX
    arg1 = "."        # 对应 EBX
)
```
...
**总结**：
这种“寄存器传参 + 软中断触发”的模式，正是操作系统 **系统调用（System Call）ABI（二进制接口）** 的标准实现方式。它相当于在底层做了一次标准化的“远程过程调用（RPC）”。
